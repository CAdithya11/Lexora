...
const Roadmap = ({ jobGoal, JsonRoadmapData, onProgressChange }) => {
  ...
  const getCompletedCountBySubCategory = (subCategoryId, mainNodeId) => {
    ...
    let completedSteps = subCategory.sub_steps.filter(
      step => completedItems.steps.includes(step.steps_id)
    ).length;
                    ...
                    <div className="p-4 border-t border-gray-100">
                      ...
                      <div className="space-y-4">
                        {mainItem.sub_category && mainItem.sub_category.map((subCategory) => {
                          ...
                          const subCategoryStats = getCompletedCountBySubCategory(subCategory.sub_id, mainItem.main_text_id);
                          const hasSteps = subCategory.sub_steps && Array.isArray(subCategory.sub_steps) && subCategory.sub_steps.length > 0;
                            ...
                            <div key={subCategory.sub_id} className="border border-gray-200 rounded-md overflow-hidden">
                              ...
                              <div className="p-4 bg-white">
                                ...
                                {hasSteps && subCategory.sub_steps.length > 1 && !isSubExpanded && (
                                  <p className="text-sm text-blue-500 cursor-pointer" onClick={(e) => {
                                    e.stopPropagation();
                                    toggleSubExpand(subCategory.sub_id);
                                  }}>
                                ...
                                {hasSteps && (
                                  <div 
                                    className={`transition-all duration-300 ${
                                      isSubExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'
                                    }`}
                                  >
                                          ...
                                          return (
                                            <li key={step.steps_id} className="flex gap-2 items-start text-sm">
                                              <div
                                                ...
                                                onClick={(e) => {
                                                  e.stopPropagation();
                                                  handleStepClick(step.steps_id, subCategory.sub_id, mainItem.main_text_id);
                                                }}
                                              >
                                                {isStepCompleted && <Check size={14} className="text-white" />}
                                              </div>
                                              <div>
                                                <p className={`${isStepCompleted ? 'text-gray-500' : 'text-gray-800'}`}>
